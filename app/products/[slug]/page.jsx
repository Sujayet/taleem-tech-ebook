    ...
    <main className={styles.page}>
      <div className="container">
        ...
        <section className={styles.layout}>
          ...
          <div>
            <div>
              <span className={styles.eyebrow}><Sparkles size={13} /> {product.categories?.name || 'Digital E-Book'}</span>
            ...
            <div className={styles.buyBox}>
              <div className={styles.priceRow}>
                ...
                {hasDiscount && <del className={styles.oldPrice}>{money(product.compare_at_price)}</del>}
              </div>
              ...
              <div className={styles.specs}>
                <div className={styles.spec}><FileText size={18} className={styles.specIcon} /><small>Format</small><b>Digital e-book</b></div>
                <div className={styles.spec}><BookOpen size={18} className={styles.specIcon} /><small>Length</small><b>{product.pages ? `${product.pages} pages` : 'Digital guide'}</b></div>
              </div>
            </div>
          </div>
        </section>
        ...
        <section className={styles.details}>
          <div className={styles.sectionHead}><div><span className={styles.eyebrow}>PRODUCT DETAILS</span><h2>About this e-book</h2></div></div>
          <div className={styles.prose}>
            <p>{product.description || 'This digital learning resource is designed to make computer learning easier, more practical, and easier to follow.'}</p>
        ...
        {related.length > 0 && <section className={styles.related}>
          <div className={styles.sectionHead}><div><span className={styles.eyebrow}>KEEP LEARNING</span><h2>Related E-Books</h2></div><Link href="/ebooks" className={styles.viewAll}>View all <ArrowRight size={16} /></Link></div>
          <div className={styles.relatedGrid}>
            {related.map((item) => <Link className={styles.relatedCard} href={`/products/${item.slug}`} key={item.id}>
              <div className={styles.relatedCover}>{item.cover_url ? <img src={item.cover_url} alt={`${item.title} cover`} /> : <BookOpen size={42} />}{item.pages && <span className={styles.pages}>{item.pages} pages</span>}</div>
              <div className={styles.relatedBody}><small>{item.categories?.name || 'Computer Skills'}</small><h3>{item.title}</h3><p>{item.description || 'Practical digital learning guide.'}</p><div className={styles.relatedPrice}><b>{money(item.price)}</b>{Number(item.compare_at_price) > Number(item.price) && <del>{money(item.compare_at_price)}</del>}</div></div>
            </Link>)}
          </div>
        </section>}
      </div>
