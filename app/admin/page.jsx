...
const emptyForm = { title: '', slug: '', description: '', price: '', compare_at_price: '', category: '', pages: '', cover_url: '', active: true, featured: false }
...
export default function Admin() {
  ...
  async function loadAdmin(showRefresh = false) {
    ...
    setProfile(p)
    const { data, error: productError } = await supabase.from('products').select('id,title,slug,description,price,compare_at_price,category,pages,cover_url,active,featured,created_at').order('created_at', { ascending: false })
  ...
  const filtered = useMemo(() => products.filter(p => {
    ...
    const matchesSearch = !q || [p.title, p.slug, p.category, p.description].some(v => String(v || '').toLowerCase().includes(q))
    const matchesFilter = filter === 'all' || (filter === 'active' && p.active) || (filter === 'inactive' && !p.active) || (filter === 'featured' && p.featured) || (filter === `cat:${p.category}`)
    return matchesSearch && matchesFilter
  }), [products, search, filter])

  function openNew() { setForm(emptyForm); setModal('new'); setError(''); setNotice('') }
  function openEdit(product) { setForm({ ...emptyForm, ...product, price: product.price ?? '', compare_at_price: product.compare_at_price ?? '', pages: product.pages ?? '' }); setModal('edit'); setError(''); setNotice('') }
  ...
  async function saveProduct(e) {
    ...
    const payload = { title: form.title.trim(), slug: form.slug.trim(), description: form.description.trim(), price: Number(form.price) || 0, compare_at_price: Number(form.compare_at_price) || 0, category: form.category.trim(), pages: Number(form.pages) || 0, cover_url: form.cover_url.trim(), active: Boolean(form.active), featured: Boolean(form.featured) }
    if (!payload.title || !payload.slug) { setError('Title and slug are required.'); setSaving(false); return }
    const result = modal === 'edit' ? await supabase.from('products').update(payload).eq('id', form.id) : await supabase.from('products').insert(payload).select().single()
