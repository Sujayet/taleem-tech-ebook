...
export default function Admin() {
  ...
  async function loadAdmin(showRefresh = false) {
    ...
    const [{ data, error: productError }, { data: categoryData, error: categoryError }] = await Promise.all([
      supabase.from('products').select('id,title,slug,description,price,compare_at_price,cover_url,image_urls,pages,featured,category_id,active,created_at,categories(id,name,slug)').order('created_at', { ascending: false }),
  ...
  async function saveProduct(e) {
    ...
    const payload = { title: form.title.trim(), slug: form.slug.trim(), description: form.description.trim(), price: Number(form.price) || 0, compare_at_price: Number(form.compare_at_price) || 0, category_id: form.category_id || null, pages: Number(form.pages) || 0, cover_url: form.cover_url.trim(), image_urls: imageUrls, active: Boolean(form.active), featured: Boolean(form.featured) }
    if (!payload.title || !payload.slug) { setError('Title and slug are required.'); setSaving(false); return }
    const result = modal === 'edit' ? await supabase.from('products').update(payload).eq('id', form.id) : await supabase.from('products').insert(payload).select('id,title,slug,description,price,compare_at_price,cover_url,image_urls,pages,featured,category_id,active,created_at,categories(id,name,slug)').single()
