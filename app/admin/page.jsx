...
export default function Admin() {
  ...
  async function loadAdmin(showRefresh = false) {
    if (showRefresh) setRefreshing(true); setError('')
    const { data: auth, error: authError } = await supabase.auth.getUser(); const currentUser = auth?.user || null; setUser(currentUser)
  ...
  async function saveProduct(e) {
    ...
    if (result.error) { setError(result.error.message || 'Could not save this e-book.'); setSaving(false); return }
    ...
    if (savedId) {
      ...
      if (galleryError) setNotice('E-book saved. Gallery images need the Supabase image_urls migration before they can be stored.')
      else setNotice(modal === 'edit' ? 'E-book updated successfully.' : 'E-book added successfully.')
    }
  ...
  async function deleteProduct(product) { if (!window.confirm(`Delete “${product.title}”? This cannot be undone.`)) return; setError(''); setNotice(''); const { error: deleteError } = await supabase.from('products').delete().eq('id', product.id); if (deleteError) setError(deleteError.message || 'Could not delete this e-book.'); else { setProducts(items => items.filter(p => p.id !== product.id)); setNotice('E-book deleted successfully.') } }
