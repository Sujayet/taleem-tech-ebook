...
import { ArrowLeft, CalendarDays, CheckCircle2, FileText, IndianRupee, Loader2, MapPin, Mail, Phone, Printer, UserRound, Wallet } from 'lucide-react'
...
function amountInWords(value){
  ...
  const chunk=(x,unit='')=>{if(!x)return '';const text=x<100?two(x):`${ones[Math.floor(x/100)]} Hundred${x%100?' '+two(x%100):''}`;return `${text}${unit?' '+unit:''} `}
  let r=''
  if(n>=10000000)r+=chunk(Math.floor(n/10000000),'Crore')
  if(n%10000000>=100000)r+=chunk(Math.floor((n%10000000)/100000),'Lakh')
  if(n%100000>=1000)r+=chunk(Math.floor((n%100000)/1000),'Thousand')
  if(n%1000)r+=chunk(n%1000)
  return `${r.trim()} Rupees Only`
}
function formatDate(value){if(!value)return '—';const d=new Date(`${value}T00:00:00`);return d.toLocaleDateString('en-IN',{day:'2-digit',month:'2-digit',year:'numeric'})}

function ReceiptContent(){
  const params=useSearchParams(), id=params.get('id')
  const [loading,setLoading]=useState(true),[error,setError]=useState(''),[payment,setPayment]=useState(null),[totalPaid,setTotalPaid]=useState(0)
  useEffect(()=>{(async()=>{
    ...
    const {data,error:e}=await supabase.from('fee_payments').select('id,amount,paid_on,method,receipt_no,notes,student_id,students(id,admission_no,full_name,course,batch,total_fee,guardian_name,join_date)').eq('id',id).single()
    if(e||!data){setError(e?.message||'Receipt not found.');setLoading(false);return}
    const {data:history,error:he}=await supabase.from('fee_payments').select('amount').eq('student_id',data.student_id)
    if(he){setError(he.message);setLoading(false);return}
    setPayment(data);setTotalPaid((history||[]).reduce((sum,row)=>sum+Number(row.amount||0),0));setLoading(false)
  })()},[id])
  ...
  const paid=Number(payment.amount||0),totalFee=Number(student.total_fee||0),balance=Math.max(totalFee-totalPaid,0)
  return <main className={styles.wrapper}>
    ...
    <section className={styles.receipt}>
      <header className={styles.header}>
        ...
        <div className={styles.brand}><div className={styles.brandName}><span>TALEEM</span> <em>TECH</em></div><h1>COMPUTER TRAINING CENTRE</h1><div className={styles.iso}>AN ISO 9001:2015 CERTIFIED</div><div className={styles.tagline}>Learn&nbsp; | &nbsp;Practice&nbsp; | &nbsp;Grow&nbsp; | &nbsp;Build Your Future</div></div>
      ...
      <div className={styles.metaStrip}><div><FileText/><span>Receipt No.<b>{payment.receipt_no||'—'}</b></span></div><div><CalendarDays/><span>Receipt Date<b>{formatDate(payment.paid_on)}</b></span></div><div><Wallet/><span>Payment Mode<b>{method}</b></span></div></div>
      <div className={styles.studentPanel}>
        <div className={styles.studentColumn}><div><UserRound/><span>Student Name<b>{student.full_name||'—'}</b></span></div><div><FileText/><span>Student ID<b>{student.admission_no||'—'}</b></span></div><div><CheckCircle2/><span>Course Name<b>{student.course||'—'}</b></span></div></div>
        <div className={styles.studentColumn}><div><MapPin/><span>Centre ID<b>{centreId}</b></span></div><div><CalendarDays/><span>Admission Date<b>{formatDate(student.join_date)}</b></span></div><div><UserRound/><span>Guardian Name<b>{student.guardian_name||'—'}</b></span></div></div>
      </div>
      <table className={styles.feeTable}><thead><tr><th>Sr.</th><th>Particulars / Fee Details</th><th>Amount (₹)</th></tr></thead><tbody>
        ...
        {payment.notes&&<tr><td>2</td><td>{payment.notes}</td><td>—</td></tr>}
      ...
      <div className={styles.balanceRow}><span>Total Course Fee: <b>{money(totalFee)}</b></span><span>Total Paid: <b>{money(totalPaid)}</b></span><span>Balance Due: <b>{money(balance)}</b></span></div>
